...
function event_trade(e)
	...
	local qglobals = eq.get_qglobals(e.other)
	if(item_lib.check_turn_in(e.trade, {item1 = 62800}) then --Redwood Seed
	...
	elseif qglobals["DruPQ"] == "1" and item_lib.check_turn_in(e.trade, {item1 = 62801}) then -- Unkempt Seed Ritual
		...
		e.other:QuestReward(e.self, {itemid=62802, exp=1}) -- Give 4 slot container Pot / reward exp, how much ?
	...
	elseif qglobals["DruPQ"] == "3" and item_lib.check_turn_in(e.trade, {item1 = 62809}) then -- Turn in Seed of Wrath
		...
		e.other:QuestReward(e.self, {itemid=62809, exp=1}) -- Give seed of wrath back - reward exp - finish 1.5 prequest.
	end	
